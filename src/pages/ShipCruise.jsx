...
export const ShipCruise = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await publicInstance.get("/listing?type=ship-cruise");
        setListings(res.data.listings || []);
      } catch (err) {
        setError("Could not load ship cruises");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div style={styles.page}>
      <div style={{ ...styles.hero, background: "#0d2137" }}>
        <h1 style={styles.heroTitle}>🚢 Ship Cruises</h1>
        <p style={styles.heroSub}>Luxury ship cruises across oceans and seas</p>
        <input
          style={styles.search}
      ...
      <div style={styles.container}>
        {loading && <p style={styles.loading}>Loading ship cruises...</p>}
        {error && <p style={styles.error}>{error}</p>}

        {!loading && filtered.length === 0 && (
          <div style={styles.empty}>
            <p style={styles.emptyIcon}>🚢</p>
            <p style={styles.emptyTitle}>No ship cruises available</p>
            <p style={styles.emptyText}>
              Check back soon or contact us for custom cruise packages.
            </p>
          </div>
        )}
