...
export const Bus = () => {
  ...
  useEffect(() => {
    const fetchBuses = async () => {
      try {
        ...
        setError(err.response?.data?.message || "Could not load bus listings");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <section className="destinations">
      <div className="title">
        <h1>Bus Trips</h1>
        <p>Browse available bus listings and book your next trip.</p>
      </div>
      ...
      {loading && <p className="loading-text">Loading bus trips...</p>}
      {error && <p className="loading-text">{error}</p>}

      <div className="destination-grid">
        {!loading && filtered.length === 0 ? (
          <p className="no-results">No bus trips available right now.</p>
        ) : (
